...
class SDN_switch: public node {
    ...
    public:
        ...
        // pass the dst , and return the size of node , sdn_node will sent pkt to
...
void SDN_switch::recv_handler (packet *p){
  ...
  if (p->type() == "SDN_ctrl_packet") { // the switch receives a packet from the controller
    ...
    unsigned int sdn_id = p3->getHeader()->getDstID(); // also it will be ourself
    //-------------------------------------------------
    ...
    setSdnTable(dst_id, next_hop, ratio); // make sdn_table (key : dst id , value : vector which save  nexthop id and ratio)
...
void SDN_controller::recv_handler (packet *p){
  ...
  if (p->type() == "SDN_ctrl_packet") { // the switch receives a packet from the controller
      ...
      // In SDN_ctrl_pkt_gen_event::trigger()
      //--------------get data--------------------------------
      unsigned int con_id = p3->getHeader()->getSrcID(); // get controller id
      unsigned int sdn_id = p3->getHeader()->getDstID(); // sdn node
      //-------------------------------------------------
      unsigned int dst_id = l3->getMatID(); // dst_id (every flow give a dst id)
      unsigned int next_hop = l3->getActID(); // send to which sdn node , when give a specific dst 
      double ratio = l3->getPer(); // how much percenrtage of flow send to next node
      //---------------------------------------------
      
      //-------------set send data----------------------------
      // p3->getHeader()->setSrcID (); // no need to change
      // p3->getHeader()->setDstID ( dst_id ); // the same
      p3->getHeader()->setPreID (getNodeID()); // set pre id = ourself
      p3->getHeader()->setNexID (sdn_id); // next node is sdn node
      
      l3->setMatID(dst_id); // when know dst_id of this pkt(flow)  
      l3->setActID(next_hop); // we will send it to next_hop
      l3->setPer(ratio);
...
// Because I do baseline , so I don't choose sdn node , so it look like Hw2
// But I still write How I choose to upgrade node
int main()
{
    ...
    // transmit upgrade_cost to compare_cost (large cost -> small cost)
    // compare_cost is a pair vector, save node : cost
    // vector<pair<unsigned int, int>> compare_cost(upgrade_cost.begin(), upgrade_cost.end());
    // sort(compare_cost.begin(), compare_cost.end(), cmp);

    // upgraded_node is a vector , save the sdn node
    // vector<unsigned int> upgraded_node;
    // vector<vector<double>> sdn_table_global;
    // int total_cost = 0;
    // for(auto it = compare_cost.begin(); it != compare_cost.end(); it++) {
    //   if(total_cost + it->second <= budget) {
    //     total_cost += it->second;
    //     upgraded_node.push_back(it->first);
    //   }
    // }
    //------------------caculate sdn node-------------------------------------------    
    ...
    //   unsigned int dst_id = dstid_list[i];
    ...
    //     vector<double> temp;
    //     sdn_node_id = *it;
    //     next_hop = next_node[sdn_node_id];
