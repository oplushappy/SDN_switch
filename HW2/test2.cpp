            ...
            public:
        	    ...
        	    static node * generate (string type, unsigned int _id) {
        	        if(id_node_table.find(_id)!=id_node_table.end()){
            		    ...
            		    node * created_node = prototypes[type]->generate(_id); // node_generator* ->
...
class TRA_switch: public node {
    ...
    public:
        ...
        // const int number_of_turn(const unsigned int src_id) {
        //     if(turn.find(src_id) == turn.end()) {
        //         return 0;
        //     } else {
        //         return turn[src_id];
        //     }
        // }
        // void Add_turn(const unsigned int src_id) {
        //     if(turn.find(src_id) == turn.end()) {
        //         turn[src_id] = 1;
        //     } else {
        //         turn[src_id]++;
...
//that is, a TRA_ctrl_packet will be generated for a source (src) with a counter 0
...
void node::send_handler(packet *p){
    ...
    e_data.s_id = _p->getHeader()->getPreID(); // sender ID
    e_data.r_id = _p->getHeader()->getNexID(); // reciever ID
    e_data._pkt = _p; // packet
    send_event *e = dynamic_cast<send_event*> (event::event_generator::generate("send_event",event::getCurTime(), (void *)&e_data) ); // I suppose this make package to send in curTime 
    if (e == nullptr) cerr << "event type is incorrect" << endl;
}
...
// All fun && var in TRA_Switch can be accessed
void TRA_switch::recv_handler (packet *p){
    ...
    // I only need to caculate count and its parent
    ...
    //------------untie pkt----------------------
        ...
        unsigned int src_id = p3->getHeader()->getSrcID();
        unsigned int pre_id = p3->getHeader()->getPreID();
        int pre_counter = l3->getCounter();
        bool send_button = false;


        if(!Is_exist_turn(src_id)) { // if first time receive packet , it will be !false
            Set_turn(src_id);
            Set_parent_counter(src_id, pre_counter);
            Create_map(src_id , pre_id);
            send_button = true;
        } 
        // if(number_of_turn(src_id) == 0) {
        //     Add_turn(src_id);
        //     Set_parent_counter(src_id, pre_counter);
        //     Create_map(src_id, pre_id);
        //     send_button = true;
        ...
        // void setPreID(unsigned int _preID) { preID = _preID ;}
        // unsigned int getNodeID() const { return id ;}
        // header * -> 
        if(send_button) {
            p3->getHeader()->setPreID ( getNodeID() ); // make message
        ...
        // l3->increase(); // counter add first
        // hi = true; // look counter and id  
        // send_handler(p3); // the action of sending
        ...
        // 1. look table
        // 2. untie packege change message (data_packet)
        ...
        // // table map...
        if(routing_table.find(p3->getHeader()->getDstID()) == routing_table.end()) { //find key whether is exist
            return;
        }
        ...
        p3->getHeader()->setPreID ( getNodeID() ); // make message
        p3->getHeader()->setNexID ( routing_table[p3->getHeader()->getDstID()] ); // table[dst_id]
        // p3->getHeader()->setDstID ( dstid );
...
int main()
{
    ...
    int nodes, dsts, links, pairs, simtime;
    
    cin >> nodes >> dsts >> links >> pairs >> simtime;
    ...
    for(int i = 0; i < dsts; i++) {
        cin >> temp_id >> temp_time;
        broadcast_time[temp_id] = temp_time;  
    }
    
    // links
    // no cin but create null first
    unsigned int node1, node2;
    for(unsigned int id = 0; id < (unsigned int)nodes; id++) {
        node::node_generator::generate("TRA_switch", id); // ?
    }
    for(int i = 0; i < links; i++) {
        cin >> temp_id >> node1 >> node2;
        // id_to_node return node ptr in id_node_table
        node::id_to_node(node1)->add_phy_neighbor(node2);
        node::id_to_node(node2)->add_phy_neighbor(node1);
    }
    
    // pairs
    unsigned int src, dst, flow_size, start_time;
    vector<vector<unsigned int>> flow_list; // (pairs, vector<unsigned int>(4,0));

    for(int flow_id = 0; flow_id < pairs; flow_id++) {
        cin >> temp_id >> src >> dst >> flow_size >> start_time;
    ...
    // node 4 sends a packet to node 0 at time 200 --> you need to implement routing tables for TRA_switches
    // data_packet_event(4, 0, 200); // data_packet is type TRA_data_packet

    for(int flow_id = 0; flow_id < pairs; flow_id++) {
        data_packet_event(flow_list[flow_id][0], flow_list[flow_id][1], flow_list[flow_id][3]);
    }
    ...
    for(int node_id = 0; node_id < nodes; node_id++) {
        cout << node_id << "\n";
        TRA_switch * temp = nullptr;
        temp = dynamic_cast<TRA_switch *> (node::id_to_node(node_id));
        for(int j = 0; j < dsts; j++) {
            cout << dstid_list[j] << " " << temp->routing_table[dstid_list[j]] << "\n";
        }
    }
